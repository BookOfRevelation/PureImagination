...
void GifSaver::process()
{
    ...
    GifBegin(w, (targetPath + "/gifresult.gif").toStdString().c_str(), firstImage.width(), firstImage.height(), 4);
    GifWriteFrame(w ,firstImage.bits(), firstImage.width(), firstImage.height(),4);
    for(int i = 1 ; i < data->getImageCount() ; ++i)
    {
        QImage curImg = data->getImage(i);
        curImg = curImg.scaled(firstImage.width(), firstImage.height());
        GifWriteFrame(w ,curImg.bits(), curImg.width(), curImg.height(),4);
    }
    GifEnd(w);
